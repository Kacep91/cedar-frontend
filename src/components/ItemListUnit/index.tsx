...
export const ItemListUnit = (props: ProductCardType) => {
  ...
  const { id } = props
  const isUnitInList = list && list.length > 0 && list.find((item: any) => item?.id === id)
  ...
  return (<>
    ...
    <ProductCard>
      ...
      <ProductBuySection>
        ...
        {isUnitInList ? <ProductBuyButton isDelete={true} onClick={() => {
          dispatch(CartActions.deleteItem({ id }))
          toast?.current?.show({
            severity: "warn",
            content: <ToastWrapper onClick={() => {
              navigate('/cart')
            }}>
          ...
          <i
            ...
            style={{ color: "#708090", width: "16px", height: "16px" }}
          ></i>
        </ProductBuyButton> : <ProductBuyButton onClick={() => {
