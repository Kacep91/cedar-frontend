...
import { emailRegex, phoneRegex, stableSort } from 'utils/utils';
    ...
    const ItemTemplate = (item: any) => {
        return (
            <OrderListItem key={item.id}>
                <i className="pi pi-times" style={{ fontSize: '2rem', cursor: 'pointer' }} onClick={() => {
                ...
                <OrderListImage src={item.image || placeHolder} style={{ objectFit: "cover" }} alt={item.name} />
                <OrderListName>
                    <span>{item.name}</span>
                ...
                <Dropdown
                    ...
                    value={item.amount}
                    onChange={(e) => {
                        dispatch(CartActions.setItem({ ...item, amount: e.target.value }))
                    }}
    ...
    return <>
        <CartWrapper>
            ...
            <CheckOrderWrapper>
                ...
                {isLoading ? <ProgressSpinner style={{ width: '50px', height: '50px' }} strokeWidth="4" animationDuration="1s" /> : <OrderListCustom style={{ width: '100%' }}>{cartList.slice(0, cartList.length)?.sort(stableSort).map(item => {
                ...
                <OrderConfirmForm style={{ width: '100%' }}>
                    ...
                    <RadioGroup>
                        {categories.map((category) => {
                            return (
                                <RadioButtonWrapper key={category.name} onChange={() => setSelectedCategory(category.name)}>
                                    <div style={{ display: 'flex' }}>
                                        <Checkbox onChange={() => setSelectedCategory(category.name)} checked={selectedCategory === category.name}></Checkbox>
